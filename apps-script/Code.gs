...
function doGet() {
  // Template mode so Index.html can <?!= include('Reference') ?>
...
// Used by Index.html to embed Reference.html and Buttons.html as static
// in-UI content tabs (no server roundtrip, no MIC-sheet write). When
// SPEC §17 changes, update the HTML files and re-deploy — deploy cycle
// equals sync cycle.
