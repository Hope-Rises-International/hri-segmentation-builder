...
def apply_tier2_suppression(
    ...
    # 166 records). SPEC §5.5.1 prescribes a forward-compatible match
    # (case-insensitive, whitespace-stripped, dash-with-or-without-space)
    # so any future Bekah picklist cleanup ("In House" / case variants /
    # collapsed dash) doesn't require a code change. Default ON.
    # Operator flips OFF only for an N-prefix in-house-only mailing —
    # handled upstream by validate_campaign_selection.
    if toggles.get("major_donor_in_house", True):
        inhouse_field = (
            accts.get("Major_Donor_In_House__c", pd.Series("", index=accts.index))
            .fillna("").astype(str).str.strip().str.lower()
        )
        inhouse_match = inhouse_field.isin({
            "major - in house",   # current live value
            "in house",           # post-cleanup (anticipated by SPEC §5.5.1)
            "major-in-house",     # dash-no-space variant
        })
